...
Все изменения в проекте RedPetroleum Backend документируются в этом файле.
...
- **Корневая причина:**
  ...
  - При cross-subdomain запросах (`redp.asystem.kg` → `ocpp.redp.asystem.kg`) браузер **не отправляет** cookies с `SameSite=Strict`
...
- **Проблема:**
  - После изменения `COOKIE_DOMAIN` с `ocpp.redp.asystem.kg` на `.redp.asystem.kg` в браузере оставались **два набора cookies**
  - Старые cookies с `Domain=ocpp.redp.asystem.kg` (невалидные)
  - Новые cookies с `Domain=.redp.asystem.kg` (валидные)
...
- **Решение:**
  ...
  - При каждом `/api/v1/auth/login` backend теперь:
    1. Удаляет старые cookies с `Domain=ocpp.redp.asystem.kg` (max_age=0)
    2. Устанавливает новые cookies с `Domain=.redp.asystem.kg`
  - Это гарантирует, что в браузере остаются только актуальные cookies
- **Код изменения:**
  ```python
  # ВАЖНО: Очищаем старые cookies с Domain=ocpp.redp.asystem.kg
  for cookie_name in ("evp_access", "evp_refresh", "XSRF-TOKEN"):
      resp.set_cookie(
          cookie_name,
          "",
          domain="ocpp.redp.asystem.kg",  # Старый domain
          max_age=0,  # Удаляем
      )

  # Устанавливаем НОВЫЕ cookies с Domain=.redp.asystem.kg
...
- **Проблема:**
  - Cookie устанавливались с `Domain=ocpp.redp.asystem.kg`
  - Фронтенд на `redp.asystem.kg` не получал cookies
...
- **Корневая причина:**
  - При логине на `https://ocpp.redp.asystem.kg/api/v1/auth/login` cookies сохранялись только для `ocpp.redp.asystem.kg`
  - При переходе на `redp.asystem.kg` браузер не отправлял эти cookies (разные домены)
  - Фронтенд не видел `evp_access` и `evp_refresh` → редирект на логин
- **Решение:**
  - Изменен `COOKIE_DOMAIN` с `ocpp.redp.asystem.kg` на `.redp.asystem.kg` (с точкой в начале)
  - Обновлены все default fallback значения в коде
  - Теперь cookies доступны для всех поддоменов: `redp.asystem.kg`, `ocpp.redp.asystem.kg`, `redp.asystem.kg`
- **Изменения в ENV:**
  ```bash
  # Было:
  COOKIE_DOMAIN=ocpp.redp.asystem.kg

  # Стало:
  COOKIE_DOMAIN=.redp.asystem.kg
  ```
- **Результат:**
  - ✅ Cookies работают на всех поддоменах `*.redp.asystem.kg`
  - ✅ Сессия сохраняется при переходе между `redp.asystem.kg` и `ocpp.redp.asystem.kg`
...
VAPID_SUBJECT=mailto:noreply@redp.asystem.kg
...
6. Smoke testing:
   ```bash
   # 1. Check VAPID key
   curl https://ocpp.redp.asystem.kg/api/v1/notifications/vapid-public-key
