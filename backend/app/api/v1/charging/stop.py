...
from app.services.push_service import push_service, get_station_owner_id, get_station_name, check_and_send_low_balance_warning
...
@router.post(
    ...
    description="Stops an active charging session. Calculates actual energy consumed, refunds unused balance reservation.",
    responses={
        ...
        404: {"description": "Session not found", "content": {"application/json": {"example": {"success": False, "error": "not_found", "message": "Сессия не найдена"}}}},
    },
)
async def stop_charging(
    ...
    try:
        ...
        db.rollback()
        logger.error(f"Ошибка баланса при остановке зарядки: {e}", exc_info=True)
        return {
            "success": False,
            "error": "balance_error",
        ...
        db.rollback()
        logger.error(f"Ошибка остановки зарядки: {e}", exc_info=True)
        return {
            "success": False,
