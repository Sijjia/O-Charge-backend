...
@router.post("/payment/h2h-payment", response_model=H2HPaymentResponse, summary="Create H2H card payment", description="Direct card payment via O!Bank H2H integration. Currently disabled.", responses=AUTH_RESPONSES)
async def create_h2h_payment(
    ...
    try:
        ...
        db.rollback()
        logger.error(f"❌ H2H payment exception: {e}", exc_info=True)
        return H2HPaymentResponse(
            success=False,
            client_id=request.client_id,
