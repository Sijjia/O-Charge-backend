...
def get_current_owner(request: Request, db: Session) -> dict:
    """
    Получить текущего owner (operator/admin/superadmin).
    Для read-only эндпоинтов, доступных всем ролям панели управления.
    """
    ...
    if not row:
        raise HTTPException(status_code=403, detail="Доступ запрещён. Пользователь не является owner.")

    if row.role not in ("operator", "admin", "superadmin"):
        raise HTTPException(status_code=403, detail="Требуется роль operator, admin или superadmin")
...
@router.get("", response_model=OperatorsListResponse, summary="List operators", description="Returns all operators with active status.", responses=ADMIN_RESPONSES)
...
@router.post("", response_model=OperatorResponse, summary="Create operator", description="Creates a new operator by phone number.", responses=ADMIN_RESPONSES)
...
@router.delete("/{operator_id}", response_model=MessageResponse, summary="Deactivate operator", description="Deactivates an operator.", responses=ADMIN_RESPONSES)
...
@router.post("/{operator_id}/activate", response_model=MessageResponse, summary="Activate operator", description="Reactivates a deactivated operator.", responses=ADMIN_RESPONSES)
