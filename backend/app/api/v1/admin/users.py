...
OwnerRole = Literal["operator", "admin", "superadmin", "partner"]
...
@router.get("", response_model=UsersListResponse, summary="List admin users", description="Returns paginated list of admin/operator users. Supports role, search, and active filtering.", responses=ADMIN_RESPONSES)
...
@router.get("/{user_id}", response_model=UserDetailResponse, summary="Get user details", description="Returns details of a specific admin/operator user.", responses=ADMIN_RESPONSES)
async def get_user(
    user_id: str,
    request: Request,
    db: Session = Depends(get_db),
...
@router.post("", response_model=UserDetailResponse, summary="Create admin user", description="Creates a new admin or operator user in Supabase Auth and the database.", responses=ADMIN_RESPONSES)
...
@router.put("/{user_id}", response_model=UserDetailResponse, summary="Update user", description="Updates user role, active status, or name.", responses=ADMIN_RESPONSES)
async def update_user(
    ...
    if data.role is not None:
        if current_role == "operator":
            logger.warning(f"Role escalation blocked: operator {current_user['id']} tried to set role={data.role} on user {user_id}")
            raise HTTPException(status_code=403, detail="Операторы не могут изменять роли")
        if current_role == "admin" and data.role in ("admin", "superadmin"):
            logger.warning(f"Role escalation blocked: admin {current_user['id']} tried to set role={data.role} on user {user_id}")
            raise HTTPException(status_code=403, detail="Админ может назначать только роль operator")
        if current_role not in ("admin", "superadmin"):
            raise HTTPException(status_code=403, detail="Недостаточно прав для изменения роли")
...
@router.delete("/{user_id}", response_model=MessageResponse, summary="Deactivate user", description="Soft-deactivates a user (sets is_active=false).", responses=ADMIN_RESPONSES)
...
@router.post("/{user_id}/activate", response_model=MessageResponse, summary="Activate user", description="Reactivates a previously deactivated user.", responses=ADMIN_RESPONSES)
