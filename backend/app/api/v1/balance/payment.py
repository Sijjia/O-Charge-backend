...
@router.post("/balance/h2h-payment", response_model=H2HPaymentResponse, summary="Create H2H card payment", description="Processes a direct card payment (host-to-host) for balance topup.", responses=AUTH_RESPONSES)
...
@router.post("/balance/token-payment", response_model=TokenPaymentResponse, summary="Create token card payment", description="Processes a payment using a saved card token.", responses=AUTH_RESPONSES)
