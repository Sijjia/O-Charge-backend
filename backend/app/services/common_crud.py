...
class CommonCrudService:
    ...
    def update_client_balance(self, client_id: str, new_balance: float) -> bool:
        ...
        try:
            ...
            logger.error(f"Ошибка обновления баланса клиента {client_id}: {e}", exc_info=True)
            return False
    ...
    def update_connector_status(self, station_id: str, connector_id: int, status: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Ошибка обновления статуса коннектора {station_id}:{connector_id}: {e}", exc_info=True)
            return False
    ...
    def create_payment_transaction(self, client_id: str, transaction_type: str, 
        ...
        try:
            ...
            logger.error(f"Ошибка создания транзакции для {client_id}: {e}", exc_info=True)
            return False