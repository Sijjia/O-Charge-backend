"""
Сервис динамического ценообразования для RedPetroleum
...
class PricingService:
    ...
    # Ночной тариф (ТЗ Red Petroleum §4.3)
    NIGHT_START = time(23, 0)  # 23:00
    NIGHT_END = time(6, 0)     # 06:00
    NIGHT_DISCOUNT = Decimal('0.20')  # -20%
    ...
    @classmethod
    def is_night_time(cls, current_time: time) -> bool:
        """Проверяет, является ли текущее время ночным (23:00-06:00)"""
        return current_time >= cls.NIGHT_START or current_time < cls.NIGHT_END

    @classmethod
    def apply_night_discount(cls, rate: Decimal) -> Decimal:
        """Применяет ночную скидку -20% к тарифу"""
        return rate * (Decimal('1') - cls.NIGHT_DISCOUNT)

    @classmethod
    def get_effective_rate(cls, base_rate: float, current_time: Optional[time] = None) -> tuple:
        """Возвращает (эффективный_тариф, применён_ли_ночной).
        Удобный метод для использования в ws_handler и других модулях.
        """
        if current_time is None:
            current_time = datetime.now(BISHKEK_TZ).time()
        is_night = cls.is_night_time(current_time)
        if is_night:
            return float(Decimal(str(base_rate)) * (Decimal('1') - cls.NIGHT_DISCOUNT)), True
    ...
    def _calculate_pricing_internal(
        ...
        if station_data['price_per_kwh'] and station_data['price_per_kwh'] > 0:
            base_rate = Decimal(str(station_data['price_per_kwh']))
            current_time = calculation_time.time()
            is_night = self.is_night_time(current_time)

            if is_night:
                rate = self.apply_night_discount(base_rate)
                rule_name = "Индивидуальный тариф станции (ночной -20%)"
                night_flag = True
            else:
                rate = base_rate
                rule_name = "Индивидуальный тариф станции"
                night_flag = False

            logger.info(f"Используем {rule_name}: {rate} {station_data['currency']}/кВт⋅ч")
            return PricingResult(
                ...
                rule_details={"type": "station_specific", "station_id": station_id, "night_discount": night_flag},
                time_based=False,
        ...
        return self._get_default_pricing(calculation_time)
    ...
    def _build_pricing_from_rule(
        ...
        # Применяем ночную скидку -20% на энергию и поминутный тариф
        current_time = calculation_time.astimezone(BISHKEK_TZ).time() if calculation_time else datetime.now(BISHKEK_TZ).time()
        if self.is_night_time(current_time):
            if pricing.rate_per_kwh > 0:
                pricing.rate_per_kwh = self.apply_night_discount(pricing.rate_per_kwh)
            if pricing.rate_per_minute > 0:
                pricing.rate_per_minute = self.apply_night_discount(pricing.rate_per_minute)
            pricing.active_rule = f"{pricing.active_rule} (ночной -20%)"
            pricing.rule_details['night_discount'] = True
    ...
    def _get_default_pricing(self, calculation_time: Optional[datetime] = None) -> PricingResult:
        """Возвращает базовый тариф с учётом ночной скидки"""
        ...
        if is_night:
            rate = self.apply_night_discount(base_rate)
            rule_name = f'Базовый тариф (ночной -20%)'
            rule_details = {'type': 'default', 'night_discount': True}
        else:
            rate = base_rate
            rule_name = 'Базовый тариф'
            rule_details = {'type': 'default', 'night_discount': False}
    ...
    def _save_pricing_history(
        ...
        """Сохраняет историю расчета тарифа (не ломает основную транзакцию)"""
        try:
