        ...
        try:
            ...
            for sub in subscriptions:
                try:
                    ...
                    logger.error(f"Failed to send push to {sub.endpoint[:50]}...: {e}", exc_info=True)
                    failed_count += 1
                    ...
                    logger.error(f"Unexpected error sending push to {sub.endpoint[:50]}...: {e}", exc_info=True)
                    failed_count += 1
    ...
    async def send_to_client(
        ...
        **kwargs
        ...
        try:
            ...
            logger.error(f"Error sending client push (event: {event_type}): {e}", exc_info=True)
    ...
    async def send_to_owner(
        ...
        **kwargs
        ...
        try:
            ...
            logger.error(f"Error sending owner push (event: {event_type}): {e}", exc_info=True)
            return {"success": False, "error": str(e)}
...
def get_station_owner_id(db: Session, station_id: str) -> Optional[str]:
    ...
    try:
        ...
        logger.error(f"Error getting station owner_id for {station_id}: {e}", exc_info=True)
        return None


def get_station_name(db: Session, station_id: str) -> str:
    """
    Получить название станции (model) или location name.

    Returns:
        str: Название станции или station_id как fallback
    """
    try:
        result = db.execute(text("""
            SELECT s.model, l.name
            FROM stations s
        ...
        if result:
            return result[0] or result[1] or station_id
        return station_id

    except Exception as e:
        logger.error(f"Error getting station name for {station_id}: {e}", exc_info=True)
        return station_id
