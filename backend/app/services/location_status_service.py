...
class LocationStatusService:
    ...
    def determine_location_status(
        ...
        - occupied: ВСЕ станции заняты (нет свободных)
        - partial: есть смесь (свободные + занятые/офлайн)
        - maintenance: ВСЕ станции на обслуживании
        - offline: ВСЕ станции офлайн или нет станций
        """
        ...
        # Есть хотя бы одна свободная — partial (даже если другие offline/occupied)
        if available_stations > 0:
            return "partial"

        # Нет свободных, но есть занятые
        if occupied_stations > 0:
            return "occupied"

        # Все на обслуживании
        if maintenance_stations == total_stations:
            return "maintenance"
