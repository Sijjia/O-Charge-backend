...
class PaymentProviderService:
    ...
    def __init__(self, force_provider: Optional[str] = None):
        ...
        elif self.provider == "NAMBA_ONE":
            from app.services.namba_service import namba_one_service
            self.service = namba_one_service
        else:  # O!Dengi (default)
    ...
    async def create_payment(
        ...
        **kwargs
        ...
        if self.provider == "OBANK":
            ...
            # Создаем одноразовую ссылку на оплату через Namba One
            response = await self.service.create_payment(
                amount=amount,
                order_id=order_id,
            ...
            return {
                ...
                "invoice_id": response.get("invoice_id", order_id),
                "link_guid": response.get("link_guid", ""),
                "provider": "NAMBA_ONE",
    ...
    async def check_payment_status(
        ...
        if self.provider == "OBANK":
            ...
            # Namba One возвращает унифицированный формат
            response = await self.service.check_payment_status(
                invoice_id=invoice_id,
                order_id=order_id
            )
            ...
            return {
                ...
                "numeric_status": response.get("numeric_status", 0),
    ...
    async def cancel_payment(
        ...
        if self.provider == "OBANK":
            ...
            # Namba One поддерживает refund через API
            webhook_url = f"{settings.DOMAIN}/api/v1/payment/webhook"
            ext_refund_id = f"refund-{transaction_id}-{uuid4().hex[:8]}"
            response = await self.service.create_refund(
                payment_order_guid=transaction_id,
            ...
            return {
                "success": response.get("success", False),
                "refund_id": response.get("refund_guid"),
    ...
    def verify_webhook(self, payload: bytes, signature: str, **kwargs) -> bool:
        ...
        if self.provider == "OBANK":
            ...
            # Namba One: верификация через custom webhook headers
            # Передаём заголовки запроса в kwargs
            request_headers = kwargs.get("request_headers", {})
            return self.service.verify_webhook(request_headers)
...
def get_namba_payment_service() -> PaymentProviderService:
    """Получение сервиса для платежей через Namba One"""
    return PaymentProviderService(force_provider="NAMBA_ONE")