...
class RealtimeService:
    ...
    async def broadcast_location_update(db: Session, location_id: str):
        ...
        try:
            ...
            logger.error(f"Ошибка отправки обновления локации {location_id}: {e}", exc_info=True)
    ...
    async def broadcast_station_update(db: Session, station_id: str):
        ...
        try:
            ...
            logger.error(f"Ошибка отправки обновления станции {station_id}: {e}", exc_info=True)
    ...
    async def broadcast_connector_update(db: Session, station_id: str, connector_id: int):
        ...
        try:
            ...
            logger.error(f"Ошибка отправки обновления коннектора {station_id}:{connector_id}: {e}", exc_info=True)
    ...
    async def broadcast_charging_session_update(db: Session, session_id: str, event_type: str):
        ...
        try:
            ...
            logger.error(f"Ошибка отправки обновления сессии {session_id}: {e}", exc_info=True)