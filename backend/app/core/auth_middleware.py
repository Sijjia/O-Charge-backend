    ...
    async def __call__(self, scope, receive, send):
        ...
        if auth_header.lower().startswith("bearer "):
            ...
            try:
                ...
                if settings.is_production:
                    if not settings.JWT_VERIFY_ISS:
                        logger.critical("JWT_VERIFY_ISS is empty in production — rejecting request")
                        return await self._unauthorized(scope, receive, send, "unauthorized",
                            "JWT issuer not configured")
                    if not settings.JWT_VERIFY_AUD:
                        logger.critical("JWT_VERIFY_AUD is empty in production — rejecting request")
                        return await self._unauthorized(scope, receive, send, "unauthorized",
        ...
        if access_cookie:
            try:
                ...
                logger.warning(f"[AUTH] Cookie JWT decode failed: {e}")
            # Если client_id найден — вызываем app ВНЕ try/except
            if client_id:
                await self.app(scope, receive, send)
                return
        ...
        PUBLIC_PATHS = [
            ...
            "/api/v1/stations",  # Список станций для мобильного приложения
            ...
            "/api/v1/auth/sms/send-otp",  # SMS OTP: отправка кода (публичный)
            "/api/v1/auth/sms/verify",    # SMS OTP: верификация (публичный - точка входа)
            "/api/v1/auth/dev-login",     # Dev Login: быстрый вход без OTP (disabled in production)
            "/api/v1/auth/dev-set-balance",  # Dev: установка баланса для тестирования
            "/api/v1/auth/sso/exchange",  # SSO: Keycloak code exchange (staff login)
            "/api/v1/guest/start",   # Гостевая зарядка: старт без авторизации
            "/api/v1/guest/pay",     # Гостевая зарядка: оплата
            "/api/v1/payment/webhook",  # Webhook от платежных провайдеров (ODENGI, OBANK, NAMBA_ONE)
        ]
        ...
        # Дополнительно: разрешаем GET для /api/v1/station/tariff/{station_id}
        if path.startswith("/api/v1/station/tariff/") and method == "GET":
            is_public = True
        ...
        # Гостевая зарядка: status и stop по session_id (аналогично charging/status)
        if path.startswith("/api/v1/guest/status/") and method == "GET":
            is_public = True
        if path.startswith("/api/v1/guest/stop/") and method == "POST":
            is_public = True
