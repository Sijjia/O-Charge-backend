...
class StationAuth:
    ...
    def __init__(self):
        self.verify_api_keys = os.getenv("VERIFY_STATION_API_KEYS", "false").lower() == "true"
    ...
    async def verify_station_connection(
        ...
        # Allow localhost connections (internal simulator)
        if websocket:
            client = websocket.client
            if client and client.host in ("127.0.0.1", "::1", "localhost"):
                logger.info(f"Station {station_id} localhost bypass — internal simulator")
                return True

        # Allow whitelisted simulator stations (SIM-* prefix) without API key
        allowed_sims = [s.strip() for s in os.getenv("SIMULATOR_STATION_IDS", "").split(",") if s.strip()]
        if station_id.startswith("SIM-"):
            if not allowed_sims or station_id in allowed_sims:
                logger.info(f"Station {station_id} simulator bypass — whitelisted")
                return True
            else:
                logger.warning(f"Station {station_id} rejected — SIM prefix but not in whitelist")
                return False
