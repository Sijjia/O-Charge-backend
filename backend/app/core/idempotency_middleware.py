...
class IdempotencyMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        if not idem_key:
            # Без Idempotency-Key повторные запросы НЕ будут дедуплицированы.
            # Логируем для мониторинга — клиент должен передавать ключ.
            logger.warning(
                f"Missing Idempotency-Key header for {method} {path}. "
                "Retry deduplication disabled for this request."
            )
            # Пропускаем middleware — выполняем запрос без idempotency
            await self.app(scope, receive, send)
            return
        ...
        try:
            ...
            async def send_wrapper(message):
                ...
                if message["type"] == "http.response.start":
                    ...
                    try:
                        ...
                        logger.debug(f"Idempotency key echo failed: {e}")
            ...
            try:
                db.execute(
                    text(
                        """
                        INSERT INTO idempotency_keys (key, method, path, body_hash, response_json, status_code, expires_at)
                        VALUES (:key, :method, :path, :body_hash, :response_json, :status_code, NOW() + INTERVAL '24 hours')
                        """
                    ),
                    {
                        "key": idem_key,
