...
class PaymentAuditMiddleware:
    ...
    PAYMENT_ENDPOINTS = {
        ...
        "/api/v1/balance/h2h-payment",
        "/api/v1/balance/token-payment",
        "/api/v1/payment/create-token",
        "/api/v1/payment/webhook",
        "/api/v1/payment/cancel",
        "/api/v1/charging/start",   # Списание средств
        "/api/v1/charging/stop"     # Возврат средств
    }
    ...
    async def __call__(self, request: Request, call_next):
        ...
        if request.method in ["POST", "PUT", "PATCH"]:
            try:
                ...
                logger.debug(f"Failed to read request body for audit: {e}")
        ...
        try:
            ...
            if path in ["/api/v1/balance/topup-card", "/api/v1/balance/h2h-payment", "/api/v1/charging/start"]:
        ...
        try:
            ...
            db.execute(text("""
                INSERT INTO payment_audit_log (
                    ...
                    :request_id, :operation_type, :client_id, :amount,
                    :client_ip, CAST(:request_data AS jsonb), CAST(:response_data AS jsonb),
                    :success, NOW()
                )
            """), {
                "request_id": request_id,
