...
class TestPricingService:
    ...
    def test_weekend_pricing(self, pricing_service, mock_db):
        ...
        with patch('app.services.pricing_service.datetime') as mock_datetime:
            # Суббота (2024-01-20 isoweekday=6)
    ...
    def test_fallback_to_default_pricing(self, pricing_service, mock_db):
        ...
        assert result.rate_per_kwh == Decimal('13.5')  # Дефолтная цена (обновлена)
...
class TestComplexScenarios:
    ...
    def test_connector_type_filtering(self, pricing_service):
        ...
        def execute_side_effect(query, params=None):
            call_count[0] += 1
            result = MagicMock()
            # Первый вызов в каждом calculate_pricing — _get_station_data
            if 'station_id' in (params or {}) and 'connector_type' not in (params or {}):
                result.fetchone.return_value = (
                    "station1", None, None, "KGS", "plan1", "План 1"
                )
                result.fetchall.return_value = []
            elif params and params.get('connector_type') == 'CCS':
                # Нет правил для CCS
        ...
        assert result.rate_per_kwh == Decimal('13.5')  # Дефолтный (обновлён)
